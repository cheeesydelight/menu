...
categorySelect.addEventListener("change", () => {
  ...
  priceWrapper.style.display =
    value !== "pizzas" && value !== "noodles" ? "block" : "none";
...
function renderMenuItem(category, id, item) {
  ...
  let actionHTML = `
    <a href="#!" onclick="editItem('${category}', '${id}', '${encodeURIComponent(
    item.name
  )}', ${item.price || 0}, '${encodeURIComponent(item.image)}', ${
    item.size7 || 0
  }, ${item.size10 || 0}, '${item.type || ""}', ${item.half || 0}, ${
    item.full || 0
  })">Edit</a>
...
function loadMenuItems() {
  ...
  categories.forEach((category) => {
    ...
    ref.on("child_added", (snap) =>
      renderMenuItem(category, snap.key, snap.val())
    );
    ref.on("child_changed", (snap) =>
      renderMenuItem(category, snap.key, snap.val())
    );
    ref.on("child_removed", (snap) =>
      document.getElementById(`card-${category}-${snap.key}`)?.remove()
    );
  });
}
...
if (saveComboBtn) {
  saveComboBtn.addEventListener("click", async () => {
    ...
    if (!name || isNaN(price))
      return alert("Enter combo name and valid price.");
...
if (searchInput) {
  searchInput.addEventListener("input", () => {
    ...
    document.querySelectorAll("#menu-items .menu-card").forEach((card) => {
      ...
      card.style.display =
        name.includes(query) || category.includes(query) ? "block" : "none";
    });
  });
}
